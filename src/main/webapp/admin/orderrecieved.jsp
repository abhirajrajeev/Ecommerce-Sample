...
try{
	...
	ResultSet rs=st.executeQuery("select * from bucket inner join product where bucket.product_id=product.id and bucket.orderdate is not NULL and bucket.status='processing'");
	while(rs.next())
	{
             ...
             <td><%=rs.getString(11)%></td>
              <td><%=rs.getString(12)%></td>
               ...
               <td><a href="cancelorderaction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1)%>">Cancel <i class='fas fa-window-close'></i></a></td>
                <td><a href="deliveredaction.jsp?id=<%=rs.getString(2)%>&email=<%=rs.getString(1)%>">Delivered <i class='fas fa-dolly'></i></i></a></td>
            </tr>
         <%	}
}
catch(Exception e)
