...
try{
	...
	ResultSet rs=st.executeQuery("select sum(total) from bucket where email='"+email+"' and status='bill'");
	while(rs.next())
	{
		total=rs.getInt(1);
	}
	ResultSet rs2=st.executeQuery("select * from cart inner join bucket where bucket.email='"+email+"' and bucket.status='bill'");
	while(rs2.next())
	{
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery("select * from bucket inner join product where bucket.product_id=product.id and bucket.email='"+email+"' and bucket.status='bill'");
  while(rs1.next())
  {
	  sno=sno+1;
